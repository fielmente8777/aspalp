...
export const useForm = <T extends BaseFormData = BaseFormData>({
  ...
  const defaultValidate = useCallback((): FormErrors => {
    ...
    return newErrors;
  }, [formData, includeCheckIn, includeCheckOut, includeMessage, singleDateMode]);
  ...
  const getDescription = useCallback((): string => {
    ...
    return parts.join("\n");
  }, [formData, includeCheckIn, includeCheckOut, includeMessage, includeCity, singleDateMode]);
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data } = await axios.post<{ Status: boolean; message?: string }>(
  ...
  }, [formData, validateForm, transformData, getDescription, createdFrom, formHid, resetForm, onSubmitSuccess]);
